...
export async function POST(request) {
  try {
    ...
    const { name, email, phone, workshopType, location, preferredDate, groupSize, additionalDetails } = body
    ...
    const safeWorkshopType = escapeHtml(workshopType)
    const safeLocation = escapeHtml(location)
    const safeGroupSize = escapeHtml(groupSize.toString())
    ...
    const { error } = await resend.emails.send({
      from: `Baked by Ann <${process.env.WORKSHOPS_FROM_EMAIL || 'onboarding@resend.dev'}>`,
      to: process.env.RECIPIENT_EMAIL || 'your-email@example.com',
      subject: `New Workshop Enquiry from ${safeName} - Baked by Ann`,
      replyTo: email,
      html: `<!DOCTYPE html>
<html>
<head>
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="field highlight">
        <div class="label">Workshop Type</div>
        <div class="value" style="font-size: 20px; font-weight: bold; color: #1F2937;">${safeWorkshopType}</div>
      </div>
      ...
      <div class="field">
        ...
        <div class="value">${safeGroupSize} people</div>
      </div>
    ...
    console.error('Workshop enquiry error:', error)
    return NextResponse.json(
